...
def transform_data(extraction_years,season_dates,this_year,lookback_years=3,branch='men',force=False):
    tranformed_filepath = f'data/transformed_data_{branch}{str(this_year)[-2:]}.csv'
    sos_scaled = False
    if os.path.exists(tranformed_filepath) and not force:
        data = pd.read_csv(tranformed_filepath)
    else:
        # Extract all cleaned data from data_extraction notebook
        if branch == 'men':
            ncaa,sportsref,bart = clean_data.clean_data(
                extraction_years,season_dates,this_year,lookback_years,branch)
        else:
            ncaa,sportsref = clean_data.clean_data(
                extraction_years,season_dates,this_year,lookback_years,branch)
